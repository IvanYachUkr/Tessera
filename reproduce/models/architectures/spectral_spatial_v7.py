...
class CenterContextReadout(nn.Module):
    """
    Explicitly preserves the center pixel, neighbour mean, and center-context contrast.
    This is much better aligned with center-cell classification than pure GAP.
    """
...
class SpatialEncoderV7(nn.Module):
...
class TemporalEncoderV7(nn.Module):
...
class PriorFeatureExtractor(nn.Module):
    """
    Builds a compact, physically meaningful summary from the raw unscaled center pixel.

    We explicitly compute a handful of classic EO indices and summarize them over time.
    The goal is not to replace the neural model with hand-written rules; the goal is to
    give the model a learnable prior branch anchored in spectral reality.
    """
    def __init__(self, n_timesteps: int = 6,
        ...
        self.eps = eps
        self.feature_dim = 7 * 6  # 7 index series × [mean, std, min, max, amp, slope]
    ...
    def _heuristic_anchor_logits(self,
        ...
        ndvi_mean, ndvi_std, ndvi_min, ndvi_max, ndvi_amp, _ = ndvi_s.unbind(dim=1)
        ndwi_mean, _, _, ndwi_max, _, _ = ndwi_s.unbind(dim=1)
        mndwi_mean, _, _, mndwi_max, _, _ = mndwi_s.unbind(dim=1)
        ndbi_mean, _, _, ndbi_max, _, _ = ndbi_s.unbind(dim=1)
        bsi_mean, _, _, bsi_max, _, _ = bsi_s.unbind(dim=1)
        ndmi_mean, _, _, ndmi_max, _, _ = ndmi_s.unbind(dim=1)
        nbr2_mean, _, _, _, _, _ = nbr2_s.unbind(dim=1)

        # These are intentionally conservative, high-precision heuristics.
        # They should be confident only on obvious pixels. Ambiguous pixels should fall
        # back to the learned branches instead of being bullied by bad rules.
        tree = (
            4.0 * (ndvi_mean - 0.58)
            + 2.0 * (ndmi_mean - 0.05)
            - 2.0 * (bsi_mean - 0.02)
        ...
        return 2.0 * logits  # V7.1: reduced from 3.0 for conservative anchors
...
class SoftThresholdPriorHead(nn.Module):
    """
    Learnable fuzzy threshold bank over spectral summary features.

    For each feature x_j and learned threshold t_jk we create soft indicators:
      sigmoid(s_jk * (x_j - t_jk))  and  sigmoid(s_jk * (t_jk - x_j))
    This gives the network an interpretable "above / below threshold" basis without
    freezing the thresholds by hand.
    """
...
class AmbiguityHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        h = self.net(x)
        amb_logit = self.ambiguity(h)
        gate_logit = self.prior_gate(h)
        return amb_logit, gate_logit


class SpectralSpatialNetV7(nn.Module):
    """
    V7: prior-guided ambiguity-aware classifier.

    Main idea:
      - keep the fast V5-style center-aware backbone
      - keep the index branch strong instead of suppressing it
      - add a spectral-prior head built from raw unscaled center-pixel summaries
      - add an ambiguity head so hard labels are trusted less on mixed pixels
      - let final logits mix neural evidence, learned prior evidence, and strong
        heuristic anchors only where the input itself justifies it
    """
    def __init__(self,
                 n_bands: int = 12,
                 n_timesteps: int = 6,
        ...
        self.temporal = TemporalEncoderV7(
        ...
        self.index_branch = ResidualMLPBranch(
            in_dim=n_indices,
        ...
        self.prior_features = PriorFeatureExtractor(
            n_timesteps=n_timesteps,
            band_map=band_map,
        )
        self.prior_head = SoftThresholdPriorHead(
            in_dim=self.prior_features.feature_dim,
        ...
        # 4 heterogeneity features: center-neighbour diff, patch std, temporal std,
        # neighbour disagreement estimate.
        ambiguity_in = temporal_dim * 2 + prior_hidden + 4
        self.ambiguity_head = AmbiguityHead(ambiguity_in, hidden_dim=96, dropout=dropout * 0.75)

        # Learn how hard the fixed heuristic anchor bias should enter the final logits.
        # V7.1: init reduced from 0.75 to 0.40 for conservative start
        self.anchor_scale = nn.Parameter(torch.tensor(0.40))
    ...
    def _heterogeneity_features(self, x: torch.Tensor) -> torch.Tensor:
        # x: (N, T, B, 3, 3) on scaled inputs. This is fine because we only need relative
        # heterogeneity / boundary-like behavior, not physical thresholds.
        center = x[:, :, :, 1, 1]
        neigh = x.reshape(x.size(0), x.size(1), x.size(2), 9)
        neigh_mean = (neigh.sum(dim=-1) - center) / 8.0
        center_neigh = (center - neigh_mean).abs().mean(dim=(1, 2))
        patch_std = x.std(dim=(-2, -1), unbiased=False).mean(dim=(1, 2))
        temporal_std = center.std(dim=1, unbiased=False).mean(dim=1)
        neigh_disagree = (neigh[:, :, :, :4].mean(dim=-1) - neigh[:, :, :, 4:8].mean(dim=-1)).abs().mean(dim=(1, 2))
        return torch.stack([center_neigh, patch_std, temporal_std, neigh_disagree], dim=1)
    ...
    def forward(self, patches: torch.Tensor, indices: torch.Tensor,
        ...
        prior_hidden, prior_logits = self.prior_head(prior_pack["summary"])
        prior_hidden = self.prior_norm(prior_hidden)
        prior_aux_logits = self.prior_aux(prior_hidden)

        amb_in = torch.cat([spatial_vec, idx_vec, prior_hidden, hetero], dim=1)
        ambiguity_logit, prior_gate_logit = self.ambiguity_head(amb_in)
        ambiguity = torch.sigmoid(ambiguity_logit).squeeze(1)
        prior_gate = torch.sigmoid(prior_gate_logit).squeeze(1)

        # V7.1: raised threshold from 0.55 to 0.72 so only very confident anchors fire
        anchor_strength = torch.clamp((prior_pack["anchor_conf"] - 0.72) / 0.28, min=0.0, max=1.0)
        final_logits = (
            neural_logits
            + prior_gate.unsqueeze(1) * prior_logits
        ...
        return {
            ...
            "prior_branch_logits": prior_logits,
            ...
            "boundary_score": torch.sigmoid(1.5 * (hetero[:, 0] + hetero[:, 1]) - 1.0),
